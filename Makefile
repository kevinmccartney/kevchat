client_build:
	docker build --build-arg KEVCHAT_API_URL=http://api.kev.chat -t kevchat_client:0.3.1 -f .\projects\client\Dockerfile .
client_run:
	docker run -p 3000:3000 kevchat_client:0.3.1
client_shell:
	docker run --rm -it --entrypoint sh kevchat_client:0.3.1
api_build:
...
docker_ecr_login:
	aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin 515966517503.dkr.ecr.us-east-2.amazonaws.com
