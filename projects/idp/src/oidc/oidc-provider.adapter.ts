...
// TODO: create indexes ahead of time (or just switch to redis/memcached)
...
class MongoAdapter implements Adapter {
  ...
  async findByUserCode(userCode) {
    const result = await this.coll()
      ...
      .find({ 'payload.userCode': userCode }, { projection: { payload: 1 } })
      .limit(1)
      .next();
  ...
  async findByUid(uid) {
    const result = await this.coll()
      .find({ 'payload.uid': uid }, { projection: { payload: 1 } })
      .limit(1)
      .next();
